...
const HexNode = memo(function HexNode({
  ...
  const baseR = diagramSize < 300 ? 21 : 26;
  const r = isActive ? baseR * 1.5 : isCompleted ? baseR * 1.25 : baseR;
  ...
  return (
    ...
    >
      ...
      {/* Step number inside — counter-rotates so text stays upright */}
      <motion.text
        x={x}
        y={y}
        textAnchor="middle"
        ...
        fontWeight={700}
        animate={{
          fontSize: isActive ? 18 : isCompleted ? 16 : 14,
          rotate: counterRotation,
        }}
        transition={{ type: "spring", stiffness: 60, damping: 20 }}
        style={{ userSelect: "none", transformOrigin: `${x}px ${y}px` }}
      >
        {index + 1}
      </motion.text>
      ...
      {isCompleted && (
        <motion.circle
          ...
          style={{ transformOrigin: `${x + r * 0.65}px ${y - r * 0.65}px` }}
        />
      )}
    </motion.g>
  );
      ...
      >
        ...
        <defs>
          {STEP_COLORS.map((color, i) => (
            <radialGradient key={i} id={`nodeGrad-${i}`}>
              <stop offset="0%" stopColor={GLOW_COLORS[i]} />
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          animate={{
            ...
            y: 0,
            transition: {
              // Enter: delay so the wheel rotation settles first, then spring in
              // Wheel spring (stiffness:60, damping:20) is ~90% settled by 400ms.
              // Exit takes ~150ms, so a 0.35s delay after exit = ~500ms total
              // after step change — wheel has landed, THEN text appears.
              type: "spring",
              stiffness: 220,
              damping: 24,
              delay: 0.35,
          ...
          exit={{
            ...
            y: -16,
            transition: {
              // Exit: fast tween so old text vanishes quickly while wheel
              // is still mid-rotation. No lingering stale content.
              duration: 0.15,
              ease: "easeIn",
            },
          }}
...
export default function FlywheelStickyHub() {
  ...
  useMotionValueEvent(smoothProgress, "change", (v) => {
    ...
    if (v < 0.05) {
      ...
      const idx = Math.min(Math.floor(normalized * 6), 5);
      setActiveStep(idx);
    }
  });
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col">
        ...
        >
          ...
          <div className="flex items-center">
            <StepDescriptionPanel activeStep={activeStep} />
          </div>
        </div>
      </div>
    </section>
